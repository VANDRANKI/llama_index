...
class PIINodePostprocessor(BaseNodePostprocessor):
    ...
    def mask_pii(self, text: str) -> Tuple[str, Dict]:
        ...
        if len(splits) < 2:
            raise ValueError(
                "Failed to parse PII masking response: expected the LLM output to "
        ...
        try:
            ...
            raise ValueError(
                "Failed to parse PII masking response: the 'Output Mapping:' "
                f"section was not valid JSON. Raw mapping text: {json_str_output!r}"
            ) from e
